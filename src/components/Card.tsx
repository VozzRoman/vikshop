...
const model: string = `?model=${item.name.split(' ').splice(1).join('_')}`
	...
	return (
		<li ref={forwardRef} className=' xl:w-1/5 lg:w-1/4 md:w-1/3 sm:w-1/2 w-full sm:p-3 pb-3 pt-3' >
			...
			<button onClick={handleAddToCart} className='text-white  w-full md:pt-1.5 md:pb-1.5 md:pl-1.5 md:pr-1.5 pt-3 pb-3 bg-orange-500 transition-all duration-300 rounded-sm mt-3 font-bold text-sm'>add to cart</button>
			</div>
			</div>
			</div>
		</li>
